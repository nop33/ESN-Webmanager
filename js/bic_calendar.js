...
$.fn.bic_calendar = function(options) {
    ...
    this.each(function(){
        ...
        if ( typeof opts.nombresMes != "undefined" )
            ...
            nombresMes = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
        ...
        if ( typeof opts.popover_options != "undefined" )
            ...
            popover_options = {placement: 'top', html: true};
        ...
        function mostrarCalendario(){
            ...
            var botonMesSiguiente = $('<td><a href="#" class="botonmessiguiente"><span class="glyphicon glyphicon-chevron-right"></a></td>');
            ...
            var botonMesAnterior = $('<td><a href="#" class="botonmesanterior"><span class="glyphicon glyphicon-chevron-left"></span></a></td>');
        ...
        function muestraDiasMes(mes, ano){
            ...
            for (var i=0; i<7; i++){
                if (i < primerDia){
                    ...
                    codigoDia += '><a style="position: relative">' + contadorDias + '<span class="badge" style="position: absolute;right: 0px;top: 0px;"></span></a></span>';
                    if (i == 6)
                        codigoDia += '</tr>';
                    contadorDias++;
                }
        ...
        function marcarEventos(mes, ano){
            var t_mes = mes + 1;
            events = _(events).sortBy('0');
            for(var i=0; i< events.length; i++) {
                var counter = 1;
                if ( events[i][0].split('/')[1] == t_mes && events[i][0].split('/')[2] == ano ){
                    if(events[i][0].charAt( 0 ) == '0'){
                        events[i][0] = events[i][0].substring(1);
                    }
                    $('#' + id_calendari + '_' + events[i][0].replace(/\//g, "_") ).addClass('event');
                    var title = events[i][1];
                    var content = events[i][4];
                    if(i+1 < events.length) {
                        if(events[i][0] == events[i+1][0]) {
                            while(events[i][0] == events[i+1][0]) {
                                title = title + ',<br>' + events[i+1][1];
                                content = content + '<hr>' + events[i+1][4];
                                counter++;
                                i++;
                                if(i+1 >= events.length) {
                                    break;
                    ...
                    $('#' + id_calendari + '_' + events[i][0].replace(/\//g, "_") + ' a' ).attr('data-original-title', title);
                    //bg
                    if ( events[i][3] )
                    ...
                    if ( events[i][2] == '' || events[i][2] == '#' ){
                        if ( events[i][4] != '' ){
                            ...
                            $('#' + id_calendari + '_' + events[i][0].replace(/\//g, "_") + ' a' ).attr('data-toggle', 'popover');
                            $('#' + id_calendari + '_' + events[i][0].replace(/\//g, "_") + ' a' ).addClass('hover_popover');
                            $('#' + id_calendari + '_' + events[i][0].replace(/\//g, "_") + ' a' ).find('.badge').html(counter);
                        } else {
                    ...
                    if ( events[i][4] ){
                        ...
                        $('#' + id_calendari + '_' + events[i][0].replace(/\//g, "_") + ' a' ).attr('data-content', content);
                    } else {
                        $('#' + id_calendari + '_' + events[i][0].replace(/\//g, "_") ).addClass('event_popover');
                        $('#' + id_calendari + '_' + events[i][0].replace(/\//g, "_") + ' a' ).attr('rel', 'popover');
                    }
            ...
            var today = '#' + id_calendari + '_' + day + '_' +  month + '_' +  d.getFullYear().toString();
            $(today).css('background', '#47a447');
            $(today + ' a').css('color', 'white');
            $(today + ' a').css('font-weight', 'bold');
            $('#' + id_calendari + ' ' + '.event_tooltip a').tooltip(tooltip_options);
            $('#' + id_calendari + ' ' + '.event_popover a').popover(popover_options);
 
            $('.hover_popover').hover( function(){
                $(this).popover('toggle');
            } );
        }
